...
export function DiscordStep({
  ...
  const t = useTranslations("DiscordStep");
    ...
    <>
      <div className="flex flex-wrap gap-2">
        <a href={invite} target="_blank" rel="noopener noreferrer" className="btn btn-brass btn-sm">
          <DiscordIcon className="size-4" /> {t("joinDiscord")}
        </a>
        {pruefbar && (
          <button
            type="button"
      ...
      {neuAnmelden && <p className="mt-3 text-sm text-cream/65">{t("needsRelogin")}</p>}
  ...
  return (
    <div className="rounded-lg border border-brass-400/40 bg-brass-500/10 p-4">
      ...
      <p className="mt-1.5 text-sm text-cream/75">{t("explanation")}</p>
